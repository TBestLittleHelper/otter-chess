...
import { formatUciAsSan, getExpectedHumanTime } from '@/lib/play/chess-utils';
    ...
    <div className="flex-grow flex flex-col min-h-0">
      ...
      <div className="p-3 px-6 lg:pt-4 bg-panel/30 space-y-2.5 lg:space-y-3">
        ...
        {/* Rating bracket sliders — control which Elo bucket Otter's
            predictions (both the Otter · Human panel and the "Moves by
            Rating" sweep's opponent side) are conditioned on for each
            side, independent of the Challenge Otter match settings.
            Defaults to the loaded PGN's WhiteElo/BlackElo headers, or the
            just-played match's ratings when there's no PGN — see
            applyDefaultAnalyzeElos in page.tsx. */}
      ...
      <div className="p-3 px-6 lg:pt-4 bg-panel/10 flex-grow flex flex-col space-y-2.5 lg:space-y-4 min-h-0">
        {/* Analysis — Otter's top predicted moves side by side with
            Stockfish's own top candidate lines (UCI MultiPV). Kept first
            in this column so it's the first thing visible in the sidebar
            below the mode header. */}
        ...
        <div className="pt-2.5 lg:pt-3">
          ...
          {/* Otter's Intuition Squares — the aux head's own from/to square
              ...
              inference but never decoded before now). Trained
              independently from the policy head, so it's a genuine second
              opinion from the model itself, not a restatement of
              topMoves[0] — flagged when the two disagree. */}
